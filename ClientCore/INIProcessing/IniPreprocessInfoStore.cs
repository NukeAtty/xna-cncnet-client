    ...
    public class IniPreprocessInfoStore
    {
        ...
        public void Load()
        {
            ...
            foreach (string key in keys)
            {
                ...
                if (!SafePath.GetFile(ProgramConstants.GetGameConfigPath(), values[0]).Exists)
                    continue;
        ...
        public bool IsIniUpToDate(string fileName)
        {
            ...
            string processedFileHash = Utilities.CalculateSHA1ForFile(SafePath.CombineFilePath(ProgramConstants.GetGameConfigPath(), fileName));
            if (processedFileHash != info.ProcessedFileHash)
                return false;

            string originalFileHash = Utilities.CalculateSHA1ForFile(SafePath.CombineFilePath(ProgramConstants.GetGameConfigBasePath(), fileName));
